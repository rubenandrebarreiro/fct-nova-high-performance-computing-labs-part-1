...
int main() {
    ...
    array<int, size_array>* host_a;
    array<int, size_array>* host_b;
    array<int, size_array>* host_c;
    array<int, size_array>* host_d;

    // The GPU memory buffers in the Device for the 4 Arrays of Integers
    // (2 Arrays for the Inputs and 2 Arrays for the Outputs)
    array<int, size_array>* dev_a;
    array<int, size_array>* dev_b;
    array<int, size_array>* dev_c;
    array<int, size_array>* dev_d;

    // Allocate GPU buffers for the 4 Arrays of Integers
    ...
    // Allocate RAM buffers for the 4 arrays of Integers
    // (2 Arrays for the Inputs and 2 Array for the Output)
    host_a = new array<int, size_array>(); // The Array for the Input no. 1 (Array A)
    host_b = new array<int, size_array>(); // The Array for the Input no. 2 (Array B)
    host_c = new array<int, size_array>(); // The Array for the Output no. 1 (Array C)
    host_d = new array<int, size_array>(); // The Array for the Output no. 2 (Array D)
    ...
    addKernel<<< (BLOCK_DIMENSIONS / THREADS_PER_BLOCK),
                  THREADS_PER_BLOCK >>>(dev_a->data(), dev_b->data(), dev_c->data(), size_array);
    ...
    cudaMemcpy(host_c->data(), dev_c->data(), size_in_bytes, cudaMemcpyDeviceToHost);
    ...
    decKernel<<< (BLOCK_DIMENSIONS / THREADS_PER_BLOCK),
                  THREADS_PER_BLOCK >>>(dev_c->data(), size_array);
    ...
    reverseKernel<<< ((BLOCK_DIMENSIONS / THREADS_PER_BLOCK) / 2),
                       THREADS_PER_BLOCK >>>(dev_c->data(), dev_d->data(), size_array);
    ...
    // GPU memory buffers of the Device to the CPU/RAM memory buffers of the Host
    cudaMemcpy(host_d->data(), dev_d->data(), size_in_bytes, cudaMemcpyDeviceToHost);
